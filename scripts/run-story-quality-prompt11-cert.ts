/**
 * Prompt 11 live certification. Fresh maximum 12 provider invocations.
 * Does not change production code. Reads the frozen dirty tree as-is.
 */
...
const OUT = path.resolve(".tmp/story-quality-real-cert/prompt11");
...
const RANKING_SECOND: CertCase = {
  id: "fast-ranking-second",
...
function readCapture(caseId: string): {
  ...
  const file = path.join(CAPTURE_DIR, `prompt11-cert-${caseId}.json`);
...
async function main(): Promise<void> {
  ...
  let firstRankingWrongNumberOne = false;

  const queue: CertCase[] = [PLAYER, PREVIEW, RANKING, BALANCED, HOOK];
  for (const spec of queue) {
    ...
    const result = await runRetentionProductionNarration({
      ...
      rejectedProposalCaptureCaseId: `prompt11-cert-${spec.id}`,
    });
    ...
    if (spec.id === "fast-ranking" && capture) {
      firstRankingWrongNumberOne = firstSpeechNamesWrongNumberOne(
        capture.narration,
        spec.numberOneName ?? "",
        spec.rankingMembers ?? [],
      );
    ...
    if (
      spec.id === "fast-ranking" &&
      firstRankingWrongNumberOne &&
      budget.canStartOptionalCase(RANKING_SECOND.expected)
    ) {
      queue.push(RANKING_SECOND);
    }
  }
  ...
  const summary = {
    ...
    totalCap: TOTAL_CAP,
    historicalPriorPrompts: { prompt9: 12, prompt10: 6, prompt11Fresh: budget.snapshot().used },
    stoppedReason,
    transportParity,
    authorityCounts: {
