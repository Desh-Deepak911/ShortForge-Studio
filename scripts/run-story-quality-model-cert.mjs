/**
 * Prompt 6 real /api/generate-script model-narration certification.
 * Artifacts stay under .tmp/ (gitignored). Rescue is not a model-quality pass.
 */
...
const MAX_REQUESTS = Number(process.env.PROMPT6_MAX_GENERATE_SCRIPT ?? 6);
const SCAFFOLD =
...
async function post(id, payload) {
  ...
  try {
    ...
    json = complete
      ? JSON.parse(complete)
      : { success: false, error: "non_json_response" };
  }
...
async function main() {
  ...
  for (const step of matrix) {
    ...
    if (
      classified.rescueEntered &&
      (step.id.startsWith("player-comeback-fast") ||
        step.id.startsWith("match-preview-fast") ||
        step.id.startsWith("top-five-fast"))
    ) {
      console.log("stop_early: baseline_entered_deterministic_rescue");
      break;
    }
  }
  ...
  const summary = {
    ...
    results: results.map((row) => ({
      ...
      verdict: row.verdict,
      callOutcome: row.verdict === "pass" ? "accepted" : "rejected_or_rescued",
