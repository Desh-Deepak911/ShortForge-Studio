...
function assertScoresInRange(validation: ReturnType<typeof validateHookCandidate>) {
...
function assertCoherent(result: RunBoundedHookRepairResult) {
  ...
  assert.equal(result.diagnostics.planFingerprint, result.activePlan.planFingerprint);
  if (result.candidate) {
    assert.equal(result.candidate.planFingerprint, result.activePlan.planFingerprint);
    assert.equal(result.candidate.strategyId, result.activePlan.strategyId);
    assert.equal(result.candidate.strategyVersion, result.activePlan.strategyVersion);
  }
  if (result.validation) {
    assert.equal(result.validation.planFingerprint, result.activePlan.planFingerprint);
  }
  if (result.selection) {
  ...
  [
    ...
    () => {
      const cases: Array<[string, string]> = [
        ["\n\n  Arsenal never looked back.\nNext.", "Arsenal never looked back."],
        ['"Arsenal are already champions." Next.', '"Arsenal are already champions."'],
        ["What if Arsenal already won?\nMore.", "What if Arsenal already won?"],
        ...
        ["A. B. C. of Arsenal changed everything. Next.", "A. B. C. of Arsenal changed everything."],
        [
          "Arsenal average 2.5 goals per game this season. Next.",
          "Arsenal average 2.5 goals per game this season.",
        ],
        ["Arsenal thought it was over... Then came March.", "Arsenal thought it was over..."],
        ["First Arsenal shock.\nSecond sentence here.", "First Arsenal shock."],
        ["Arsenal never blinked\nSecond line continues.", "Arsenal never blinked"],
        ["Arsenal never blinked once all season", "Arsenal never blinked once all season"],
      ];
...
await runTests("opening-limits", [
  [
    ...
    () => {
      ...
      const exactV = validateHookCandidate({ request, plan, candidate: exact });
      ...
      const overDurationWords = Math.floor(maxSeconds * HOOK_SPEAKING_WORDS_PER_SECOND) + 1;
...
await runTests("validation", [
  ...
  [
    ...
    () => {
      ...
      for (const narration of cases) {
        ...
        const { request, plan } = planFor({
          topic: narration.includes("Arsenal") ? "Arsenal title race" : "Player form",
          groundingInput: { claims: [verifiedClaim()], unavailableResearch: false },
        });
  ...
  [
    ...
    () => {
      ...
      assert.equal(tactical.reasons.includes("safety.harmful_targeting"), false);
      ...
      assert.equal(watching.reasons.includes("safety.prompt_injection"), false);
  ...
  [
    ...
    () => {
      ...
      assert.ok(superlative.reasons.includes("safety.unsupported_superlative"));
    },
  ],
]);
...
await runTests("selection-tamper", [
  [
    ...
    () => {
      ...
      const selection = buildHookSelection({ request, plan, candidate: good });
      assert.equal(selection.narrationCommitRule, "opening_span_of_narration");
    ...
    async () => {
      ...
      const result = await runBoundedHookRepair({
        ...
        compatibilityFallback: async ({ plan: fallbackPlan }) => {
          assert.equal(fallbackPlan.strategyId, HOOK_COMPATIBILITY_FALLBACK_STRATEGY_ID);
      ...
      assert.equal(result.activePlan.strategyId, HOOK_COMPATIBILITY_FALLBACK_STRATEGY_ID);
      assert.notEqual(result.activePlan.planFingerprint, plan.planFingerprint);
      assert.equal(
        result.activePlan.planFingerprint,
...
await runTests("repair-workflow", [
  ...
  [
    ...
    async () => {
      ...
      assert.equal(emptyRepair.diagnostics.validationOutcome, "generation_failed");
  ...
  [
    ...
    async () => {
      ...
      assert.equal(emptyFallback.diagnostics.validationOutcome, "generation_failed");
  ...
  [
    ...
    async () => {
      ...
      assert.equal(scenesResult.diagnostics.validationOutcome, "generation_failed");
...
await runTests("terminal-coherence-7c2", [
  [
    ...
    async () => {
      ...
      assert.ok(unavailable.diagnostics.fallbackReason?.includes("unavailable"));
      ...
      const empty = await runBoundedHookRepair({
        ...
        plan,
        narration: `${words(20)}.`,
        compatibilityFallback: async () => ({ narration: "  ", claimRefs: [] }),
      });
      ...
      assert.notEqual(salvagedValidation.candidate!.planFingerprint, originalFp);
      ...
      const success = await runBoundedHookRepair({
        ...
        compatibilityFallback: async ({
          ...
          assert.equal(fbPlan.strategyId, HOOK_COMPATIBILITY_FALLBACK_STRATEGY_ID);
          return { narration: FALLBACK_OPENING, claimRefs: [] };
        },
      });
  ...
  [
    ...
    async () => {
      ...
      assert.equal(withoutFallback.diagnostics.validationOutcome, "generation_failed");
      ...
      const whitespace = await runBoundedHookRepair({
        ...
        compatibilityFallback: async ({ previousFailureReason }) => {
          assert.equal(previousFailureReason, "initial_narration_whitespace_only");
          return { narration: FALLBACK_OPENING, claimRefs: [] };
        },
      });
  ...
  [
    ...
    async () => {
      ...
      assert.notEqual(failedBeforeCandidate.activePlan.planFingerprint, originalFp);
