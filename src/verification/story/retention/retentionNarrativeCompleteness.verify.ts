...
async function main(): Promise<void> {
  ...
  await check("[C1] spoken-completeness version is stable", () => {
    assert.equal(
      RETENTION_SPOKEN_COMPLETENESS_VERSION,
      "spoken-completeness/1",
    );
  });

  await check(
    "[C2] exact reproduced fragments fail spoken completeness",
    async () => {
  ...
  await check(
    "[C3] unterminated phrase and Because-the fragment fail",
    async () => {
      const env = await coherentEnvelope("cheap");
      const n = env.plan.beatPlan.beats.length;
      const texts = Array.from({ length: n }, (_, i) =>
  ...
  await check(
    "[C5] word-by-word style overlong candidate fails closed sentence-safe",
    async () => {
      const env = await coherentEnvelope("cheap");
      const n = env.plan.beatPlan.beats.length;
  ...
  await check(
    "[C6] sentence-safe trim preserves opening and complete utterances when possible",
    async () => {
      const env = await coherentEnvelope("cheap");
      const n = env.plan.beatPlan.beats.length;
  ...
  await check(
    "[C7] invisible beat spans may form one continuous sentence",
    async () => {
      const env = await coherentEnvelope("cheap");
      const n = env.plan.beatPlan.beats.length;
      const clauses = [
        "Why does Argentina's press matter?",
        "Because England draw the first wave forward,",
        "the space behind begins to open",
        "while Argentina keep stepping higher,",
        "turning one tactical risk into mounting pressure,",
        "and that is where the contest finally turns.",
      ];
      const texts = Array.from(
        { length: n },
        (_, index) =>
          clauses[index] ?? "and the pressure keeps building naturally,",
      );
      texts[n - 1] = "and that is where the contest finally turns.";
      const candidate = candidateFromTexts(
        env.plan,
        env.grounding,
        env.strategySeed,
      ...
      assert.ok(
        candidate.segments
          .slice(1, -1)
          .some((segment) => !/[.!?…]$/u.test(segment.text)),
      );
      const result = evaluateRetentionSpokenCompleteness(candidate, env.plan);
      assert.equal(result.ok, true, result.reasonIds.join(","));
    },
