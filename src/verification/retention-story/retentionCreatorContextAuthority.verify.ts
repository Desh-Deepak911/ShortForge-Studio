...
async function main(): Promise<void> {
  ...
  await check("[C1] 35s Studio brief with manual context → Pass", async () => {
    ...
    if (result.ok) {
      ...
      assert.equal(
        JSON.stringify(result.approved).includes(REALISTIC_35_CONTEXT.slice(0, 40)),
        false,
      );
    }
  });

  await check("[C2] Creative Premise + manual context → Pass with premise facts", async () => {
    const result = await runRetentionProductionNarration({
  ...
  await check("[C3] Creative Premise + structured Smart Research → Pass", async () => {
  ...
  await check("[C4] Smart Research with no creator context → manual identity null", () => {
    const assembled = minimalAssembled(RESEARCH_PROSE);
  ...
  await check("[C5] Resolved research prose is not hashed as manual context", () => {
    const assembled = minimalAssembled(RESEARCH_PROSE);
  ...
  await check("[C6] Creator manual + research keep separate authorities", () => {
    const assembled = minimalAssembled(RESEARCH_PROSE);
    ...
    assert.ok(grounding.claims.some((c) => c.provenance === "research_provider"));
    assert.ok(grounding.claims.some((c) => c.provenance === "manual_user"));
  });

  await check("[C7] Validation receives same context authority as planning", async () => {
    const authority = buildRetentionCreatorContextAuthority({
      manualContext: REALISTIC_35_CONTEXT,
    });
    ...
    const bridge = readyBridgeWithAuthority(candidate, env.plan, "cheap", env.ledger);
    ...
    assertRetentionCreatorContextAuthorityMatchesContract(authority, env.contract);
  });

  await check("[C8] Validation with null context when contract expects context → fail closed", async () => {
    const authority = buildRetentionCreatorContextAuthority({
      manualContext: REALISTIC_35_CONTEXT,
    });
    const env = await coherentEnvelope("cheap", {
  ...
  await check("[C9] Validation with changed context → fail closed", async () => {
    const authority = buildRetentionCreatorContextAuthority({
      manualContext: REALISTIC_35_CONTEXT,
    });
    const env = await coherentEnvelope("cheap", {
  ...
  await check("[C10] Validation with creator/research contexts swapped → fail closed", async () => {
    const authority = buildRetentionCreatorContextAuthority({
      manualContext: REALISTIC_35_CONTEXT,
    });
    const env = await coherentEnvelope("cheap", {
  ...
  await check("[C13] Post-rewrite validation retains context authority", async () => {
    const authority = buildRetentionCreatorContextAuthority({
      manualContext: REALISTIC_35_CONTEXT,
    });
  ...
  await check("[C14] JSON/NDJSON contain no raw context authority", async () => {
    const result = await runRetentionProductionNarration({
      topic: "Argentina versus England match review",
      durationSec: 30,
      generationPath: "script_only",
  ...
  await check("[C16] Concurrent research/manual requests do not leak context", async () => {
  ...
  await check("[C17] Audio-first starts VO only after corrected validation/commit", () => {
    ...
    const fullFnStart = audio.indexOf("export async function generateAudioFirstStory");
  ...
  await check("[C18] Safe failure classification is not editorial quality_failure", () => {
    assert.equal(
