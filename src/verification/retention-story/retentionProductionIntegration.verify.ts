...
function assertEnvelopeRejects(
  ...
  snapshot: ReturnType<typeof deriveSafeHookEnvelopeFromAuthority>["hookPlanSnapshot"],
  diagnostics: ReturnType<typeof deriveSafeHookEnvelopeFromAuthority>["hookDiagnostics"],
...
function assertPass(result: RetentionProductionNarrationResult): asserts result is Extract<
...
async function main(): Promise<void> {
  ...
  await check("[P2] audio-first uses Retention orchestrator; no parallel Hook-only path", () => {
    ...
    assert.match(src, /Voiceover never begins before the Retention commit gate/);
  });

  await check("[P3] route does not call Hook bridge / old generator directly", () => {
  ...
  await check("[P4] Fast script-only terminal Pass", async () => {
    ...
    assert.equal(
      result.approved.safeDiagnostics.budget?.planner,
      0,
    );
  ...
  await check("[P5] Balanced script-only Pass with exactly one planner call", async () => {
    let plannerCalls = 0;
    ...
    assert.ok(
      (result.approved.safeDiagnostics.budget?.total ?? 99) <= 6,
    );
  });

  await check("[P6] Studio script-only can Pass (rewrite path available)", async () => {
    let rewriteCalls = 0;
    const result = await runRetentionProductionNarration({
      topic: "Spain versus France tactical preview",
      durationSec: 30,
  ...
  await check("[P7] full audio-first ordering — VO/scenes only after commit", async () => {
    let voCalls = 0;
    let sceneCalls = 0;
  ...
  await check("[P8] composer throw recovers via reliability fallback before VO", async () => {
    let voCalls = 0;
    let sceneCalls = 0;
    const result = await generateAudioFirstStory({
      prompt: "Spain versus France tactical preview",
  ...
  await check("[P9] empty composer recovers via deterministic reliability fallback", async () => {
    const result = await runRetentionProductionNarration({
      topic: "Spain versus France tactical preview",
      durationSec: 30,
      generationPath: "script_only",
  ...
  await check("[P10] planner malformed output uses deterministic reliability plan", async () => {
    const result = await runRetentionProductionNarration({
      topic: "Spain versus France tactical preview",
      durationSec: 30,
      generationPath: "script_only",
  ...
  await check("[P12] scenes-only preserves narration byte-for-byte and skips Retention", async () => {
  ...
  await check("[P13] explicit Hook Style + Write My Own contract identities", () => {
  ...
  await check("[P14] template advisory + research off + manual context unverified", () => {
  ...
  await check("[P20] concurrent requests do not share ledger/plan/evidence", async () => {
  ...
  await check("[P21] script-only service success carries Retention + Hook envelopes", async () => {
  ...
  await check("[P22] research-off contract has null research identity when empty", () => {
    const input = buildProductionStoryContractInput({
      topic: "Spain versus France tactical preview",
      durationSec: 30,
      generationPath: "script_only",
  ...
  await check("[H2] pass_without_rewrite rejects rewrite-history ledger", () => {
  ...
  await check("[H3] max planner/composer schema output policies for 15/30/45/60", () => {
    for (const durationSec of [15, 30, 45, 60] as const) {
      ...
      assert.ok(plannerTokens >= RETENTION_PRODUCTION_OUTPUT_TOKEN_BOUNDS.min);
    }
  ...
  await check("[H4] voice throw / unresolved duration / scene failure strip snapshots", async () => {
    const base = {
      prompt: "Spain versus France tactical preview",
      sceneCount: 3,
      duration: 30,
    ...
    if (!voiceThrow.success) {
      ...
      assert.equal(voiceThrow.retentionEnvelope?.validationSummary, undefined);
    ...
    if (!badDuration.success) {
      ...
      assert.equal(badDuration.retentionEnvelope?.validationSummary, undefined);
    }
  ...
  await check("[H5] JSON/NDJSON failure parity — no plan/validation on success:false", async () => {
    ...
    for (const blob of [JSON.stringify(jsonEnvelope), JSON.stringify(ndjsonError)]) {
  ...
  await check("[H6] safe Hook diagnostics privacy + enum negatives", () => {
    ...
    const rejectedBlob = JSON.stringify({
      success: false,
      error: "The opening didn't pass Hook quality checks. Please try again or adjust Hook style.",
  ...
  await check("[H8] successful Fast/Balanced/Studio script-only + audio-first", async () => {
  ...
  await check("[H9] scenes-only byte-for-byte preservation (10F.3A)", async () => {
  ...
  await check("[H10] pass_after_rewrite commit-gate positive + ledger rewrite rejects", async () => {
  ...
  await check("[H11] public Retention barrel omits commit-gate/envelope helpers", () => {
    const root = readFileSync(
      path.join(ROOT, "features/retention-story/index.ts"),
      "utf8",
    );
    assert.equal(root.includes("assertCommitGateLedgerAuthority,"), false);
    assert.equal(root.includes("assertRetentionSafeHookEnvelopeCoherence,"), false);
    assert.equal(root.includes("resolveRetentionProductionMaxOutputTokens,"), false);
  });
  ...
  await check("[H12A] Balanced planner — missing/wrong strategy fields falls back to deterministic plan", async () => {
    await assertPlannerFallsBack("missing strategy", { beats: [] });
    await assertPlannerFallsBack("null strategy", { strategy: null, beats: [] });
  });

  await check("[H12A] Balanced planner — unsupported emotion falls back to deterministic plan", async () => {
  ...
  await check("[H12A] Balanced planner — invalid purpose / wrong first/terminal falls back to deterministic plan", async () => {
    const contract = normalizeStoryContract({
      topic: "Spain versus France tactical preview",
      durationSec: 30,
      qualityMode: "balanced",
  ...
  await check("[H12A] Balanced planner — beat count outside range falls back to deterministic plan", async () => {
    const contract = normalizeStoryContract({
      topic: "Spain versus France tactical preview",
      durationSec: 30,
      qualityMode: "balanced",
  ...
  await check("[H12A] Balanced planner — factual contribution without eligible support falls back to deterministic plan", async () => {
    const contract = normalizeStoryContract({
      topic: "Spain versus France tactical preview",
      durationSec: 30,
      qualityMode: "balanced",
    ...
    await assertPlannerFallsBack("factual without claim refs", { ...base, beats });
  });

  await check("[H12A] Balanced planner — syntactically valid JSON failing semantic normalize", async () => {
  ...
  await check("[H12B] Fast quality — declarative cold-open clears curiosity without question mark", async () => {
  ...
  await check("[H12C] Explicit non-question Hook style Passes without requiring question punctuation", async () => {
    const declarativeComposer: RetentionComposerCallback = (request) => {
      const n = request.orderedBeatIds.length;
      const budget = Math.round(request.durationSec * 2.4);
      const targetTotal = Math.min(
  ...
  await check("[H12C] Explicit Hook Style forwards requestedStrategyId into Hook context", async () => {
    const result = await runRetentionProductionNarration({
      topic: "Spain versus France tactical preview",
      durationSec: 30,
      generationPath: "script_only",
    ...
    assert.equal(contract.identities.hookStyleIdentity, "provocative_question");
    assert.equal(
  ...
  await check("[H12C] Explicit Hook + composer throw recovers with fallback disposition", async () => {
    const result = await runRetentionProductionNarration({
      topic: "Spain versus France tactical preview",
      durationSec: 30,
      generationPath: "script_only",
  ...
  await check("[H12D] Probe A — explicit Hook fail after compose → zero-model Auto success", async () => {
