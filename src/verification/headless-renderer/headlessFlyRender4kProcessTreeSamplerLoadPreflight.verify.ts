/**
 * Sprint 11E Phase 2E.2D.8K.4.1 — provider-free sampler load preflight entrypoint.
 */

import { runHosted4kNodeSamplerLoadPreflight } from "./fly-render-4k-capacity/hosted-render-machine-node-sampler-load-preflight";

async function main() {
  const result = await runHosted4kNodeSamplerLoadPreflight();
  console.log(
    `\n4K sampler load preflight: ${result.overall}` +
