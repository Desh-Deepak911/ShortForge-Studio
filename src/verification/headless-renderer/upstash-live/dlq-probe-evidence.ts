/**
 * Targeted Upstash DLQ probe evidence — never overwrites progressive/official.
 */
...
import {
  isDlqAttributionReasonId,
  isDlqAttributionStageId,
  scrubDlqAttributionStages,
...
export function defaultUpstashDlqProbeEvidencePath(
  ...
  return path.join(cwd, UPSTASH_DLQ_PROBE_EVIDENCE_RELATIVE_PATH);
}

export type UpstashDlqProbeEvidenceDocument = {
  ...
  readonly dlqTransport?: "tcp_xadd" | null;
  readonly notes: readonly string[];
};

export const DLQ_PROBE_EVIDENCE_TITLE =
  "Sprint 11E Phase 2D.1G — Upstash targeted DLQ probe" as const;

export const DLQ_PROBE_ELIGIBILITY = Object.freeze({
  PASS: "ELIGIBLE — targeted DLQ probe passed; progressive/official not rerun.",
  FAIL: "NOT ELIGIBLE — targeted DLQ probe failed or incomplete.",
  FAIL_CLEANUP: "NOT ELIGIBLE — targeted DLQ probe cleanup failed.",
  FAIL_CONFIG:
    "NOT ELIGIBLE — HEADLESS_UPSTASH_QA_DLQ_PROBE=1 but Neon/Upstash config missing or invalid.",
  NOT_TESTED:
    "NOT ELIGIBLE — targeted Upstash DLQ probe has not been executed.",
} as const);

export function createNotTestedUpstashDlqProbeEvidence(
  ...
  return {
    ...
    overall: "NOT_TESTED",
    eligibilityVerdict: DLQ_PROBE_ELIGIBILITY.NOT_TESTED,
    startedAtIso: null,
    endedAtIso: null,
    failureStage: null,
...
export function renderUpstashDlqProbeEvidenceMarkdown(
  doc: UpstashDlqProbeEvidenceDocument,
): string {
  const lines: string[] = [
    `# ${esc(doc.title)}`,
    "",
    ...
    `**dlqTransport:** ${
      doc.dlqTransport != null ? `\`${esc(doc.dlqTransport)}\`` : "n/a"
    }`,
...
export function writeUpstashDlqProbeEvidence(
  doc: UpstashDlqProbeEvidenceDocument,
  evidencePath: string = defaultUpstashDlqProbeEvidencePath(),
): void {
  mkdirSync(path.dirname(evidencePath), { recursive: true });
  writeFileSync(
    evidencePath,
    renderUpstashDlqProbeEvidenceMarkdown(doc),
    "utf8",
  );
}

export function preserveOrInitializeUpstashDlqProbeEvidence(
  evidencePath: string = defaultUpstashDlqProbeEvidencePath(),
): {
...
export function dlqProbeCannotFalsePass(
  doc: UpstashDlqProbeEvidenceDocument,
  ...
  try {
    ...
    if (
      ...
      !isDlqAttributionReasonId(doc.failureReasonId)
    ) {
