/**
 * Targeted Upstash claim/ACK probe evidence — never overwrites official live matrix.
 */
...
import {
  isClaimAckAttributionReasonId,
  isClaimAckAttributionStageId,
  scrubClaimAckAttributionStages,
...
export const UPSTASH_CLAIM_ACK_PROBE_EVIDENCE_RELATIVE_PATH =
  "docs/evidence/headless/current/HEADLESS_11E_UPSTASH_CLAIM_ACK_PROBE.md";

export function defaultUpstashClaimAckProbeEvidencePath(
  cwd: string = process.cwd(),
): string {
  return path.join(cwd, UPSTASH_CLAIM_ACK_PROBE_EVIDENCE_RELATIVE_PATH);
}

export type UpstashClaimAckProbeEvidenceDocument = {
  ...
  /** Bounded — never stream keys, URLs, tokens, delivery/job IDs. */
  ...
  readonly runScopedCleanupStatus?:
    ...
    | "not_run"
    | null;
  /** Bounded transport split — never stream keys / URLs / tokens / IDs. */
  readonly enqueueTransport?: "rest_xadd" | null;
  readonly consumeTransport?: "tcp_production_protocol" | null;
  readonly notes: readonly string[];
};

export const CLAIM_ACK_PROBE_EVIDENCE_TITLE =
  "Sprint 11E Phase 2D.1E — Upstash targeted claim/ACK probe" as const;

export const CLAIM_ACK_PROBE_ELIGIBILITY = Object.freeze({
  PASS: "ELIGIBLE — targeted claim/ACK probe passed; official matrix not rerun.",
  FAIL: "NOT ELIGIBLE — targeted claim/ACK probe failed or incomplete.",
  FAIL_CLEANUP: "NOT ELIGIBLE — targeted claim/ACK probe cleanup failed.",
  FAIL_CONFIG:
    "NOT ELIGIBLE — HEADLESS_UPSTASH_QA_CLAIM_ACK_PROBE=1 but Neon/Upstash config missing or invalid.",
  NOT_TESTED:
    "NOT ELIGIBLE — targeted Upstash claim/ACK probe has not been executed.",
} as const);

export function createNotTestedUpstashClaimAckProbeEvidence(
  notes: readonly string[] = [
    "Gate off — no Neon or Upstash connection attempted.",
    "Prior PASS/FAIL claim/ACK-probe evidence must not be overwritten by gate-off runs.",
  ...
  return {
    title: CLAIM_ACK_PROBE_EVIDENCE_TITLE,
    overall: "NOT_TESTED",
    eligibilityVerdict: CLAIM_ACK_PROBE_ELIGIBILITY.NOT_TESTED,
    startedAtIso: null,
    endedAtIso: null,
    failureStage: null,
...
export function renderUpstashClaimAckProbeEvidenceMarkdown(
  doc: UpstashClaimAckProbeEvidenceDocument,
): string {
  const lines: string[] = [
    `# ${esc(doc.title)}`,
    "",
...
export function writeUpstashClaimAckProbeEvidence(
  doc: UpstashClaimAckProbeEvidenceDocument,
  evidencePath: string = defaultUpstashClaimAckProbeEvidencePath(),
): void {
  mkdirSync(path.dirname(evidencePath), { recursive: true });
  writeFileSync(
    evidencePath,
    renderUpstashClaimAckProbeEvidenceMarkdown(doc),
    "utf8",
  );
}
 ...
 */
export function preserveOrInitializeUpstashClaimAckProbeEvidence(
  evidencePath: string = defaultUpstashClaimAckProbeEvidencePath(),
): {
...
export function claimAckProbeCannotFalsePass(
  doc: UpstashClaimAckProbeEvidenceDocument,
  ...
  try {
    ...
    if (
      ...
      !isClaimAckAttributionReasonId(doc.failureReasonId)
    ) {
