/**
 * Sprint 11E Phase 2D.1E — claim/ACK probe gate/evidence authority (no provider).
 * Run: npm run test:headless-upstash-claim-ack-probe-authority
 */

import assert from "node:assert/strict";
import { mkdtempSync, readFileSync, writeFileSync } from "node:fs";
import { tmpdir } from "node:os";
import path from "node:path";

import type { HeadlessSqlExecutor } from "@/features/headless-renderer/control-plane/runtime/sql-client";
import {
  createFakeUpstashRestClient,
  MemoryHeadlessJobStoreAdapter,
  MemoryHeadlessOwnedObjectStoreAdapter,
  MemoryHeadlessProjectOwnershipAdapter,
  MemoryHeadlessStreamQueueAdapter,
} from "@/features/headless-renderer/control-plane/testing";

import { runUpstashClaimAckProbe } from "../upstash-live/claim-ack-probe";
import {
  CLAIM_ACK_ATTRIBUTION_REASON_IDS,
  CLAIM_ACK_ATTRIBUTION_STAGE_IDS,
} from "../upstash-live/claim-ack-attribution";
import {
  claimAckProbeCannotFalsePass,
  createNotTestedUpstashClaimAckProbeEvidence,
  preserveOrInitializeUpstashClaimAckProbeEvidence,
  renderUpstashClaimAckProbeEvidenceMarkdown,
  writeUpstashClaimAckProbeEvidence,
} from "../upstash-live/claim-ack-probe-evidence";
import { buildUpstashLiveSchemaFingerprint } from "../upstash-live/evidence-authority";

let passed = 0;

async function test(name: string, fn: () => void | Promise<void>) {
  await fn();
  passed += 1;
  console.log(`  ✓ ${name}`);
}

function noopSql(): HeadlessSqlExecutor {
  const client = {
    async query<T = Record<string, unknown>>() {
      return { rows: [{ n: "0" }] as T[] };
    },
  };
  return {
    async withClient(fn) {
      return fn(client as never);
    },
    async withTransaction(fn) {
      return fn(client as never);
    },
  };
}

async function main() {
  console.log("\nSprint 11E Phase 2D.1E — Upstash claim/ACK probe authority\n");

  await test("gate-off opens zero connections and preserves PASS", async () => {
    const dir = mkdtempSync(path.join(tmpdir(), "uq-ca-gate-"));
    const evidencePath = path.join(dir, "probe.md");
    writeFileSync(
      evidencePath,
      renderUpstashClaimAckProbeEvidenceMarkdown({
        ...createNotTestedUpstashClaimAckProbeEvidence(),
        overall: "PASS",
        eligibilityVerdict: "prior",
        cleanupStatus: "ok",
        stages: [{ stageId: "production_ack", ok: true }],
      }),
      "utf8",
    );
    const before = readFileSync(evidencePath, "utf8");
    let connections = 0;
    const result = await runUpstashClaimAckProbe({
      env: {},
      evidencePath,
      connectionProbe: () => {
        connections += 1;
      },
    });
    assert.equal(result.exitCode, 0);
    assert.equal(result.overall, "NOT_TESTED");
    assert.equal(result.connectionFactoryCalls, 0);
    assert.equal(connections, 0);
    assert.equal(readFileSync(evidencePath, "utf8"), before);
    const preserved = preserveOrInitializeUpstashClaimAckProbeEvidence(
      evidencePath,
    );
    assert.equal(preserved.overall, "PASS");
    assert.equal(preserved.action, "preserved");
  });

  await test("gate-off initializes NOT_TESTED when absent", async () => {
    const dir = mkdtempSync(path.join(tmpdir(), "uq-ca-init-"));
    const evidencePath = path.join(dir, "probe.md");
    const result = await runUpstashClaimAckProbe({ env: {}, evidencePath });
    assert.equal(result.exitCode, 0);
    assert.equal(result.connectionFactoryCalls, 0);
    assert.ok(readFileSync(evidencePath, "utf8").includes("NOT_TESTED"));
  });

  await test("injected happy path PASS + cleanup ok (temp evidence)", async () => {
    const dir = mkdtempSync(path.join(tmpdir(), "uq-ca-pass-"));
    const evidencePath = path.join(dir, "probe.md");
    const stream = new MemoryHeadlessStreamQueueAdapter({
      envName: "staging",
      nowMs: () => 1_700_000_000_000,
    });
    await stream.ensureConsumerGroups();

    const result = await runUpstashClaimAckProbe({
      forceGateOn: true,
      assumeConfigured: true,
      evidencePath,
      env: {
        HEADLESS_UPSTASH_QA_CLAIM_ACK_PROBE: "1",
        HEADLESS_ENV_NAME: "staging",
        DATABASE_URL:
          "postgresql://user:pass@ep-test.us-east-1.aws.neon.tech/neondb",
        UPSTASH_REDIS_REST_URL: "https://example.upstash.io",
        UPSTASH_REDIS_REST_TOKEN: "token_value_0123456789",
        UPSTASH_REDIS_TCP_URL: "rediss://default:pass@example.upstash.io:6379",
      },
      injectedSql: noopSql(),
      injectedJobStore: new MemoryHeadlessJobStoreAdapter(),
      injectedOwnedObjectStore: new MemoryHeadlessOwnedObjectStoreAdapter(),
      injectedProjectAuthorization: new MemoryHeadlessProjectOwnershipAdapter(),
      injectedTcpConsumer: stream,
      injectedRestClient: createFakeUpstashRestClient(stream.testingFake()),
      injectedFingerprint: buildUpstashLiveSchemaFingerprint(),
      cleanupRunner: async () => "ok",
      nowIso: () => "2026-07-21T00:00:00.000Z",
    });
    assert.equal(result.exitCode, 0, `expected PASS exit 0 got ${result.overall}`);
    assert.equal(result.overall, "PASS");
    const md = readFileSync(evidencePath, "utf8");
    assert.ok(md.includes("**Overall:** PASS"));
    assert.equal(md.includes("UPSTASH_REDIS_REST_TOKEN"), false);
  });

  await test("missing stages / hostile cannot false PASS", () => {
    const bad = createNotTestedUpstashClaimAckProbeEvidence();
    assert.equal(claimAckProbeCannotFalsePass(bad).ok, false);
    assert.equal(
      claimAckProbeCannotFalsePass({
        ...bad,
        overall: "PASS",
        cleanupStatus: "ok",
        stages: [],
      }).ok,
      false,
    );
    assert.equal(
      claimAckProbeCannotFalsePass({
        ...bad,
        overall: "PASS",
        cleanupStatus: "failed",
        stages: [{ stageId: "production_ack", ok: true }],
      }).ok,
      false,
    );
    assert.equal(
      claimAckProbeCannotFalsePass({
        ...bad,
        overall: "PASS",
        cleanupStatus: "ok",
        stages: [{ stageId: "production_ack", ok: false }],
      }).ok,
      false,
    );
    assert.equal(
      claimAckProbeCannotFalsePass({
        ...bad,
        overall: "PASS",
        cleanupStatus: "ok",
        failureStage: "production_ack",
        stages: [{ stageId: "production_ack", ok: true }],
      }).ok,
      false,
    );
  });

  await test("unknown stage cannot produce PASS", () => {
    const doc = {
      ...createNotTestedUpstashClaimAckProbeEvidence(),
      overall: "PASS" as const,
      cleanupStatus: "ok" as const,
      stages: [
        {
          stageId: "made_up_stage" as never,
          ok: true as const,
        },
      ],
    };
    assert.equal(claimAckProbeCannotFalsePass(doc).ok, false);
  });

  await test("claim/ACK attribution allowlists are frozen", () => {
    assert.equal(CLAIM_ACK_ATTRIBUTION_STAGE_IDS.length, 10);
    assert.equal(CLAIM_ACK_ATTRIBUTION_REASON_IDS.length, 14);
    assert.ok(CLAIM_ACK_ATTRIBUTION_STAGE_IDS.includes("production_ack"));
    assert.ok(
      CLAIM_ACK_ATTRIBUTION_REASON_IDS.includes(
        "prior_run_entry_not_finalized",
      ),
    );
  });

  await test("evidence write does not touch official live path", () => {
    const dir = mkdtempSync(path.join(tmpdir(), "uq-ca-iso-"));
    const evidencePath = path.join(dir, "probe.md");
    writeUpstashClaimAckProbeEvidence(
      createNotTestedUpstashClaimAckProbeEvidence(),
      evidencePath,
    );
    assert.equal(evidencePath.includes("UPSTASH_LIVE_EVIDENCE"), false);
    assert.ok(evidencePath.includes("probe.md"));
  });

  console.log(`\n${passed} tests passed.\n`);
}

main().catch((err) => {
  console.error(err);
  process.exitCode = 1;
});
