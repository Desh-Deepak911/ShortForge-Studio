...
function buildRewritePrompt(request: RetentionBodyRewriteRequest): string {
  ...
  return [
    ...
    "Improve body quality only for the named failed dimensions. Do not perform a generic rewrite.",
    "Write one continuous narration. Preserve facts, Hook opening authority, ranking membership, and uncertainty.",
    "Required JSON shape:",
    '{ "title": string, "narration": string, "usedContentIds": string[], "omittedContentIds": string[], "hookClaimRefs": string[] }',
    ...
    `failedSafeDimensions: ${request.failedSafeDimensions.join(", ") || "(none)"}`,
