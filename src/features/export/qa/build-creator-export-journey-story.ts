/**
 * Deterministic in-memory story for the creator-export journey QA harness.
...
/** Stable in-memory project / draft identity for InspectorContext.storyId. */
export const CREATOR_EXPORT_QA_DRAFT_ID = "creator-export-qa" as const;

/** Existing repo-public SVG served by Next from `/public`. */
export const CREATOR_EXPORT_QA_IMAGE_URL = "/file.svg" as const;

export const CREATOR_EXPORT_QA_SCENE_1_ID = "cex-qa-scene-1" as const;
export const CREATOR_EXPORT_QA_SCENE_2_ID = "cex-qa-scene-2" as const;
export const CREATOR_EXPORT_QA_SCENE_3_ID = "cex-qa-scene-3" as const;

export const CREATOR_EXPORT_QA_SCENE_1_ITEM_IDS = [
  "cex-qa-scene-1-item-1",
  "cex-qa-scene-1-item-2",
  "cex-qa-scene-1-item-3",
] as const;

export const CREATOR_EXPORT_QA_SCENE_2_ITEM_IDS = [
  "cex-qa-scene-2-item-1",
  "cex-qa-scene-2-item-2",
] as const;

export const CREATOR_EXPORT_QA_SCENE_3_ITEM_ID =
  "cex-qa-scene-3-item-1" as const;

export const CREATOR_EXPORT_QA_SCENE_1_DURATION_MS = 6_000 as const;
export const CREATOR_EXPORT_QA_SCENE_2_DURATION_MS = 6_000 as const;
export const CREATOR_EXPORT_QA_SCENE_3_DURATION_MS = 5_000 as const;
export const CREATOR_EXPORT_QA_SCENE_1_WINDOW_MS = 2_000 as const;
export const CREATOR_EXPORT_QA_SCENE_2_WINDOW_MS = 3_000 as const;

/** Non-identity freeform adjustment seeded on Scene 2 item 2. */
export const CREATOR_EXPORT_QA_SCENE_2_ITEM_2_BRIGHTNESS = 110 as const;

const SCENE_1_NARRATION =
  "The midfielder receives under pressure near the halfway line. He turns quickly toward the far touchline. One sharp pass finds the runner in behind.";
...
function qaImageMedia(input: {
  ...
  return {
    type: "image",
    url: CREATOR_EXPORT_QA_IMAGE_URL,
    source: "upload",
...
function assertLocalMedia(media: SceneMedia, label: string): void {
  const url = media.url ?? "";
  if (url !== CREATOR_EXPORT_QA_IMAGE_URL) {
    throw new Error(`${label}: expected local ${CREATOR_EXPORT_QA_IMAGE_URL}.`);
  }
...
function assertCleanBaselineMedia(media: SceneMedia, label: string): void {
  ...
  // Keep export-tree scanners free of subject-aware authoring tokens.
  const record = media as unknown as Record<string, unknown>;
  const bannedKeys = [
    "subject" + "Focus",
    "subjectAware" + "FramingProvenance",
    "sourceQuality" + "AdjustmentProvenance",
  ] as const;
  for (const key of bannedKeys) {
    if (record[key] != null) {
      throw new Error(`${label}: must not seed authoring metadata.`);
    }
  }
}
...
function buildScene1(): FootieScene {
  return buildSceneFromItems({
    id: CREATOR_EXPORT_QA_SCENE_1_ID,
    durationMs: CREATOR_EXPORT_QA_SCENE_1_DURATION_MS,
    startMs: 0,
    narration: SCENE_1_NARRATION,
    subtitle: "Creator export QA · scene 1",
    itemIds: CREATOR_EXPORT_QA_SCENE_1_ITEM_IDS,
    windowDurationMs: CREATOR_EXPORT_QA_SCENE_1_WINDOW_MS,
    mediaItems: [
...
function buildScene2(): FootieScene {
  const windowMs = CREATOR_EXPORT_QA_SCENE_2_WINDOW_MS;
  ...
  const item2 = qaImageMedia({
    ...
    visualAdjustments: {
      version: 1,
      brightness: CREATOR_EXPORT_QA_SCENE_2_ITEM_2_BRIGHTNESS,
      contrast: 100,
      saturation: 100,
    },
  });
  ...
  const scene = buildSceneFromItems({
    id: CREATOR_EXPORT_QA_SCENE_2_ID,
    durationMs: CREATOR_EXPORT_QA_SCENE_2_DURATION_MS,
    startMs: CREATOR_EXPORT_QA_SCENE_1_DURATION_MS,
    narration: SCENE_2_NARRATION,
    subtitle: "Creator export QA · scene 2",
    itemIds: CREATOR_EXPORT_QA_SCENE_2_ITEM_IDS,
...
function buildScene3(): FootieScene {
  return buildSceneFromItems({
    id: CREATOR_EXPORT_QA_SCENE_3_ID,
    durationMs: CREATOR_EXPORT_QA_SCENE_3_DURATION_MS,
    startMs:
      CREATOR_EXPORT_QA_SCENE_1_DURATION_MS +
      CREATOR_EXPORT_QA_SCENE_2_DURATION_MS,
    narration: SCENE_3_NARRATION,
    subtitle: "Creator export QA · scene 3",
    itemIds: [CREATOR_EXPORT_QA_SCENE_3_ITEM_ID],
    windowDurationMs: CREATOR_EXPORT_QA_SCENE_3_DURATION_MS,
    mediaItems: [
      qaImageMedia({ width: 1080, height: 1920, fitMode: "cover" }),
    ],
  });
...
/** Deep-clone a story for harness-local in-memory checkpoints (no storage). */
export function cloneCreatorExportQaCheckpoint(
  story: FootieScript,
): FootieScript {
  return structuredClone(story);
}

/** Fresh in-memory FootieScript for the local creator-export journey QA harness. */
export function buildCreatorExportJourneyStory(): FootieScript {
  const scene1 = buildScene1();
  const scene2 = buildScene2();
  const scene3 = buildScene3();
  const story = syncFootieScript({
    title: "QA — Creator Export Disposable — Local Only",
  ...
  if (story.voiceoverUrl != null || story.voiceoverDurationMs != null) {
    throw new Error("Creator export QA story must not include voiceover.");
  }
  if (story.backgroundMusic?.enabled) {
    throw new Error("Creator export QA story must keep background music disabled.");
  }
  if (story.visualRetentionExtensions != null) {
    throw new Error(
      "Creator export QA story must not seed visualRetentionExtensions.",
    );
  }
  if (story.visualRetentionPresetProvenance != null) {
    throw new Error(
      "Creator export QA story must not seed preset provenance.",
    );
  }
  ...
  if (story.scenes.length !== 3) {
    throw new Error("Creator export QA story must contain exactly three scenes.");
  }
