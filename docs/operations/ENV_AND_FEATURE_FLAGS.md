...
`visual-beat-density-v1` remains off unless the phase list includes `12A,12B,12C` under the same staging fail-closed rules and the commit ref is an allowlisted staging-development branch (`staging`, `sprint12-staging-compat-safety-foundation`, `sprint12c-staging-visual-beat-density`, `staging-source-quality-intelligence`, `staging-keyframed-motion-overlays`, or `staging-visual-retention-presets`). Arbitrary feature branches are rejected. Default and unresolved states are disabled; main and production stay hard-off. The capability is narration-driven visual pacing only — music never enters beat calculations or requirements, and Browser export remains available even when Headless is preferred. Creator UI reads `visualBeatDensityEnabled` from the same capabilities API response (one request); no density UI or beat generation is activated by this plumbing alone.

### Visual motion capabilities (keyframed effects, engagement overlays, brand sting, subject-aware reframing)

Four independently negotiated creator booleans remain off unless the phase list is exactly ordered through `12E` (`12A,12B,12C,12D,12E`) under the same staging fail-closed rules and the commit ref is allowlisted (`staging` or `staging-keyframed-motion-overlays`, among other prior allowlisted staging-development branches). Capability IDs: `keyframed-visual-effects-v1`, `engagement-overlays-v1`, `shortforge-brand-sting-v1`, `subject-aware-reframing-v1`. Creator UI reads `keyframedVisualEffectsEnabled`, `engagementOverlaysEnabled`, `shortForgeBrandStingEnabled`, and `subjectAwareReframingEnabled` from the same capabilities API response (one request). No motion/CTA/outro/subject-framing UI is activated by this plumbing alone. Do not set `SHORTFORGE_STAGING_VISUAL_RETENTION_PHASES=12A,12B,12C,12D,12E` until later gated slices are ready.

### Visual-retention presets (`12F`)

**Current live staging** remains phases through `12E` (`12A,12B,12C,12D,12E`). No live environment change was performed for `12F`; `visualRetentionPresetsEnabled` stays false until an authorized later activation.

`visual-retention-presets-v1` remains off unless the phase list is exactly ordered through `12F` (`12A,12B,12C,12D,12E,12F`) under the same staging fail-closed rules and the commit ref is allowlisted (`staging`, `staging-visual-retention-presets`, or other prior allowlisted staging-development branches). Creator UI reads `visualRetentionPresetsEnabled` from the same capabilities API response (one request; API version remains `1`). This capability is creator-authoring orchestration for future orchestration of existing authoring settings — it does not itself become an ExportManifest renderer requirement and is not part of Browser/Headless supported-renderer capability registries. Creator Templates are unrelated generation-time templates. This slice does not ship a preset catalog, persistence, commands, UI, QA harness, or live rollout. Do not set `SHORTFORGE_STAGING_VISUAL_RETENTION_PHASES=12A,12B,12C,12D,12E,12F` until later gated slices are ready. Do not mutate any live environment variable from this documentation.
...
- `HEADLESS_R2_QA_TARGETED=1` — required with `DATABASE_URL` + R2 configured for `npm run test:headless-r2-targeted` (minimum chain; stop-on-first-failure; writes `docs/evidence/headless/current/HEADLESS_11E_R2_TARGETED_EVIDENCE.md` only)
...
- `HEADLESS_UPSTASH_QA_ENQUEUE_PROBE=1` — required with `DATABASE_URL` + REST+TCP + `HEADLESS_ENV_NAME=staging` for `npm run test:headless-upstash-enqueue-probe` (writes `docs/evidence/headless/current/HEADLESS_11E_UPSTASH_ENQUEUE_PROBE.md` only; never overwrites official live evidence)
- `HEADLESS_UPSTASH_QA_TERMINAL_PROBE=1` — required with `DATABASE_URL` + REST+TCP + `HEADLESS_ENV_NAME=staging` for `npm run test:headless-upstash-terminal-probe` (writes `docs/evidence/headless/current/HEADLESS_11E_UPSTASH_TERMINAL_PROBE.md` only; never overwrites official live evidence). PASS SHA `efb22955…` preserved.
- `HEADLESS_UPSTASH_QA_CLAIM_ACK_PROBE=1` — required with `DATABASE_URL` + REST+TCP + `HEADLESS_ENV_NAME=staging` for `npm run test:headless-upstash-claim-ack-probe` (writes `docs/evidence/headless/current/HEADLESS_11E_UPSTASH_CLAIM_ACK_PROBE.md` only; never overwrites progressive/official evidence). PASS SHA `74fc6c23…` preserved.
- `HEADLESS_UPSTASH_QA_DLQ_PROBE=1` — required with `DATABASE_URL` + REST+TCP + `HEADLESS_ENV_NAME=staging` for `npm run test:headless-upstash-dlq-probe` (writes `docs/evidence/headless/current/HEADLESS_11E_UPSTASH_DLQ_PROBE.md` only; never overwrites progressive/official evidence). PASS SHA `c448d9e2…` preserved.
...
- Proposed Fly secrets listed in [HEADLESS_11E_PHASE2E1_HOSTED_WORKER_FOUNDATION.md](../architecture/headless/HEADLESS_11E_PHASE2E1_HOSTED_WORKER_FOUNDATION.md) — **not set**
...
- `HEADLESS_FLY_VERIFY_QA=1` — required with canonical **eleven-key** QA bridge secrets + `HEADLESS_ENV_NAME=staging` + `HEADLESS_FLY_STAGING_APP_NAME=shortforge-hw-staging-4def8fa0` for `npm run test:headless-fly-verify-live` (writes `docs/evidence/headless/current/HEADLESS_11E_FLY_VERIFY_LIVE_EVIDENCE.md` only; never overwrites verify-first PASS evidence)
- `HEADLESS_FLY_RENDER_QA=1` — separate gate for `npm run test:headless-fly-render-live` (writes `docs/evidence/headless/current/HEADLESS_11E_FLY_RENDER_LIVE_EVIDENCE.md`; never activates render Machines; requires `verify=1/render=1` topology from separately authorized render activation)
...
- Intra-scene transitions: `src/features/scene-media-transitions/` · [INTRA_SCENE_TRANSITIONS.md](../product/INTRA_SCENE_TRANSITIONS.md)
