...
| **2C.1A** | R2 durability + live-evidence authority: retain `004`, Neon owned-object adapter, TOCTOU revision bind, upload capability truth, coverage reconcile, gated `HEADLESS_R2_QA` harness — **IMPLEMENTED / CONFIGURATION-GATED / LIVE NOT RUN** | See `docs/architecture/headless/HEADLESS_11E_R2_DURABILITY_EVIDENCE.md`; production routes still blocked |
...
| **2E.1** | Fly hosted-worker deployment preflight + container/runtime foundation — **IMPLEMENTED LOCALLY** | See `docs/architecture/headless/HEADLESS_11E_PHASE2E1_HOSTED_WORKER_FOUNDATION.md`; seams block consumer start; Fly deploy NOT STARTED |
| **2E.1A** | Corrections: Node 24, process-specific VMs, repo-root Dockerfile path, deterministic BUILD_INFO, `foundation_image` — **IMPLEMENTED LOCALLY** | See `docs/archive/sprints/headless/HEADLESS_11E_PHASE2E1A_CORRECTION.md`; Fly deploy NOT STARTED |
| **2E.2A … 2E.2A.3** | Streamed R2; durable pre-upload; fail-closed delete saga; terminal cleanup dispositions (`protected`/`rejected` no-delete); Neon `005` (+ `object_id` + terminal states); storage+cleanup seams closed locally — **IMPLEMENTED LOCALLY** | See `docs/architecture/headless/HEADLESS_11E_PHASE2E2A_RENDER_STORAGE_CLEANUP.md`; foundation_image retained; no remote migration; Fly deploy NOT STARTED |
| **2E.2B** | Trusted verify→promote→enqueue under dual-lease claim; `VERIFY_PROMOTION_COMPOSITION_SEAM` closed locally — **IMPLEMENTED LOCALLY** | See `docs/architecture/headless/HEADLESS_11E_PHASE2E2B_TRUSTED_VERIFY_PROMOTION.md`; packaging still blocks consumer loop; foundation_image / not deployable; Fly deploy NOT STARTED |
| **2E.2B.1** | Signal-armed shutdown; incremental asset verify; claimed-hook catch; queued dispatch recovery — **IMPLEMENTED LOCALLY** (superseded dispatch path in 2E.2B.2) | See `docs/archive/sprints/headless/HEADLESS_11E_PHASE2E2B1_HOSTED_EXECUTION_CORRECTION.md` |
| **2E.2B.2** | Durable render-dispatch outbox (`006`); atomic promote+intent; outbox claim→XADD→dispatched; unified busy/shutdown — **IMPLEMENTED LOCALLY** | See `docs/architecture/headless/HEADLESS_11E_PHASE2E2B2_DURABLE_DISPATCH_OUTBOX.md`; migration 006 not applied remotely; foundation_image / not deployable; Fly deploy NOT STARTED |
| **2E.2B.3** | Outbox CAS truthfulness — exhaustive release/reject/dispatched confirmation; promotion+outbox rollback proof — **IMPLEMENTED LOCALLY** | See `docs/architecture/headless/HEADLESS_11E_PHASE2E2B3_DISPATCH_CAS_TRUTHFULNESS.md`; packaging NOT STARTED; Fly deploy NOT STARTED |
